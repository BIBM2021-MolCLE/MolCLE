...
class GINNet(nn.Module):
    def __init__(self, input_dim, output_dim, model_args):
        ...
        if self.num_mlp_layers > 1:
            self.mlp.append(nn.Linear(self.dense_dim * len(self.readout_layers),
                                       model_args.mlp_hidden[0]))
            for i in range(1, self.num_mlp_layers-1):
                self.mlp.append(nn.Linear(self.mlp_hidden[i-1], self.mlp_hidden[1]))
            self.mlp.append(nn.Linear(self.mlp_hidden[-1], output_dim))
        else:
            self.mlp.append(nn.Linear(self.dense_dim * len(self.readout_layers),
                                       output_dim))
