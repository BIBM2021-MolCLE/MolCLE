...
def get_dataset(data_args):
    ...
    molecule_net_dataset_names = [name.lower() for name in MoleculeNet.names.keys()] + ['zinc_standard_agent']
...
def load_MolecueNet(data_args):
    ...
    # dataset = MoleculeDataset("datasets/" + data_args.dataset_name, dataset=data_args.dataset_name)
    if data_args.dataset_name != 'zinc_standard_agent':
        ...
        dataset = MoleculeNet(root=data_args.dataset_dir, name=data_args.dataset_name)
        dataset.data.x = dataset.data.x.float()
        dataset.data.y = None        
...
def get_dataloader(dataset, data_args, train_args):
    ...
    """
    if data_args.scaffold_split:
        smiles_list = pd.read_csv('datasets/' + data_args.dataset_name + '/processed/smiles.csv', header=None)[0].tolist()
        train, eval, test = scaffold_split(dataset, smiles_list, null_value=0, frac_train=0.8,frac_valid=0.1, frac_test=0.1)        
