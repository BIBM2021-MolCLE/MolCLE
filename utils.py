...
class PlotUtils():
    ...
    def plot_soft_edge_mask(self, graph, edge_mask, top_k, un_directed, nodename, edgename, figname, **kwargs):
        ...
        elif self.dataset_name.lower() in ['bbbp', 'mutag', 'zinc_standard_agent']:
            x = kwargs.get('x')
            nodelist, edgelist = self.get_topk_edges_subgraph(edge_index, edge_mask, top_k, un_directed)
            np.save(nodename, np.array(nodelist))
            np.save(edgename, np.array(edgelist))
